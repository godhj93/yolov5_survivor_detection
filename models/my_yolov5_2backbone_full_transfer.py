...
class build_yolov5s_2backbone(nn.Module):
    def __init__(self, cfg='models/yolov5s.yaml', ch=6, nc=1):
        ...
        # Load pretrained model (yolov5s) for transfer learning (backbone_RGB and head)
        weights_transfer = 'yolov5s.pt'
        cfg_transfer = 'models/yolov5s.yaml'
        nc_transfer = 1
        hyp_transfer = 'data/hyps/hyp.scratch-low.yaml'
        if isinstance(hyp_transfer, str):
            with open(hyp_transfer, errors='ignore') as f:
                hyp_transfer = yaml.safe_load(f)  # load hyps dict
        resume_transfer = False
        ckpt_transfer = torch.load(weights_transfer, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
        # print('ckpt_transfer: ', ckpt_transfer)
        model_transfer = Model(cfg_transfer or ckpt_transfer['model'].yaml, ch=3, nc=nc_transfer, anchors=hyp_transfer.get('anchors'))# .to(device)  # create
        model_transfer.eval()
        exclude_transfer = ['anchor'] if (cfg_transfer or hyp_transfer.get('anchors')) and not resume_transfer else []  # exclude keys
        csd_transfer = ckpt_transfer['model'].float().state_dict()  # checkpoint state_dict as FP32
        csd_transfer = intersect_dicts(csd_transfer, model_transfer.state_dict(), exclude=exclude_transfer)  # intersect
        model_transfer.load_state_dict(csd_transfer, strict=False)  # load
        LOGGER.info(f'Transferred {len(csd_transfer)}/{len(model_transfer.state_dict())} items from {weights_transfer}')  # report
        
        # Load pretrained model (yolov5s) for transfer learning (backbone_IR)
        weights_transfer_IR = 'yolov5s.pt'
        cfg_transfer_IR = 'models/yolov5s.yaml'
        nc_transfer_IR = 1
        hyp_transfer_IR = 'data/hyps/hyp.scratch-low.yaml'
        if isinstance(hyp_transfer_IR, str):
            with open(hyp_transfer_IR, errors='ignore') as f:
                hyp_transfer_IR = yaml.safe_load(f)  # load hyps dict
        resume_transfer_IR = False
        ckpt_transfer_IR = torch.load(weights_transfer_IR, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
        # print('ckpt_transfer: ', ckpt_transfer)
        model_transfer_IR = Model(cfg_transfer_IR or ckpt_transfer_IR['model'].yaml, ch=3, nc=nc_transfer_IR, anchors=hyp_transfer_IR.get('anchors'))# .to(device)  # create
        model_transfer_IR.eval()
        exclude_transfer_IR = ['anchor'] if (cfg_transfer_IR or hyp_transfer_IR.get('anchors')) and not resume_transfer_IR else []  # exclude keys
        csd_transfer_IR = ckpt_transfer_IR['model'].float().state_dict()  # checkpoint state_dict as FP32
        csd_transfer_IR = intersect_dicts(csd_transfer_IR, model_transfer_IR.state_dict(), exclude=exclude_transfer_IR)  # intersect
        model_transfer_IR.load_state_dict(csd_transfer_IR, strict=False)  # load
        LOGGER.info(f'Transferred {len(csd_transfer_IR)}/{len(model_transfer_IR.state_dict())} items from {weights_transfer_IR}')  # report
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[0]             # transfer 
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[1]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = model_transfer.model[2]             # transfer 
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[3]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer.model[4]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[5]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer.model[6]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[7]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer.model[8]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[9]             # transfer 
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[0]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[1]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer_IR.model[2]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[3]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer_IR.model[4]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[5]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer_IR.model[6]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[7]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer_IR.model[8]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer_IR.model[9]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   SPPF
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[14]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[18]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer.model[20]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params   
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        m_ = m(*args)
        #print(type(m_))
        m_ = model_transfer.model[21]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type:   Conv
        np = sum(x.numel() for x in m_.parameters())   # number of parameters
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number of parameters
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        ...
        n = 1
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        m_ = model_transfer.model[23]             # transfer 
        t = str(m)[8:-2].replace('__main__.', '')  # module type: C3
        np = sum(x.numel() for x in m_.parameters())  # number params   
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
