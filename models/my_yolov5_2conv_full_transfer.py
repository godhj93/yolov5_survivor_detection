...
class build_yolov5s_2conv(nn.Module):
    def __init__(self, cfg='models/yolov5s.yaml', ch=6, nc=1):
        ...
        conv_RGB = [[-1, 1, Conv, [64, 6, 2, 2]],   # 0
                    [-1, 1, Conv, [128, 3, 2]]      # 1
                    ]
        conv_IR = [[-1, 1, Conv, [64, 6, 2, 2]],    # 2
                   [-1, 1, Conv, [128, 3, 2]]       # 3
                   ]
        backbone = [[[1, 3], 1, Concat, [1]],       # 4
                    [-1, 3, C3, [128]],             # 5
        ...
        backbone_head = conv_RGB + conv_IR + backbone + head
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        # print('forward self.save: ', self.save)
        # RGB input
        m = self.model[0]
        # print('m.i: ', m.i)
        # print('m.f: ', m.f)
        ...
        if visualize:
            feature_visualization(x, m.type, m.i, save_dir=visualize)
        for m_i, m in enumerate(self.model[3:4]):
            # print(m)
            # print('m.i: ', m.i)
            m.i = m_i+3
            # print('m.f: ', m.f)
        ...
        for m_i, m in enumerate(self.model[5:]):
            # print(m)
            m.i = m_i+5
            # print('m.i: ', m.i)
            # print('m.f: ', m.f)
