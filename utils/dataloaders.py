...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            ...
            if '*' in p:
                ...
                files_IR.extend(sorted(glob.glob(os.path.join(p, 'images_IR', '*.*'))))  # dir
        ...
        images_IR = [x for x in files_IR if x.split('.')[-1].lower() in IMG_FORMATS]
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            img0 = np.concatenate([img0, img_IR], axis=2)
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        try:
            f_IR = []  # IR image files
            path_add = [path, '_IR']
            path_IR = ''.join(path_add)
            for p in path_IR if isinstance(path_IR, list) else [path_IR]:
                p = Path(p)  # os-agnostic
                if p.is_dir():  # dir
                    f_IR += glob.glob(str(p / '**' / '*.*'), recursive=True)
                elif p.is_file():  # file
                    with open(p) as t:
                        t = t.read().strip().splitlines()
                        parent = str(p.parent) + os.sep
                    ...
                    raise FileNotFoundError(f'{prefix}{p} does not exist')
            self.im_IR_files = sorted(x.replace('/', os.sep) for x in f_IR if x.split('.')[-1].lower() in IMG_FORMATS)
            assert self.im_IR_files, f'{prefix}No images found'
        except Exception as e:
        ...
        if self.rect:
            ...
            self.im_IR_files = [self.im_IR_files[i] for i in irect]  ### my modification ###
        ...
        self.npy_IR_files = [Path(f_IR).with_suffix('.npy') for f_IR in self.im_IR_files]  ### my modification ###
        if cache_images:
    ...
    def load_image(self, i):
        ...
        im, f, fn, f_IR, fn_IR = self.ims[i], self.im_files[i], self.npy_files[i], self.im_IR_files[i], self.npy_IR_files[i]
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im_IR = cv2.resize(im_IR, [640, 640])
                im_6ch = np.concatenate([im, im_IR], axis=2)
            ...
            if r != 1:  # if sizes are not equal
                ...
                im_IR = cv2.resize(im_IR, (int(w0 * r), int(h0 * r)), interpolation=interp)
                im_6ch = np.concatenate([im, im_IR], axis=2)
            return im_6ch, (h0, w0), im.shape[:2]  # im, hw_original, hw_resized
        else:
